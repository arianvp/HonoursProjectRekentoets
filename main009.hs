...
commR :: Rule
commR (Mul x (Mul u v)) = Just $ Mul u (Mul x v)
commR _                 = Nothing

fracR :: Rule
fracR (Mul l@(Div x y) r) = Just $ Mul (Double (eval l)) r
fracR (Mul l r@(Div x y)) = Just $ Mul l (Double (eval r))
fracR _           = Nothing
...
findCtx expr lhs | null candidates  = Nothing
	...
	where subs       = subExprR [distR, evalR, commR, fracR] $ toCtx expr
